  ...
  provisioner "local-exec" {
    command    = <<EOF
        curl -X POST '${local.lightlytics_api_url}/graphql' \
          ...
          -d '{"query":"mutation AccountAcknowledge($input: AccountAckInput){\r\n accountAcknowledge(account: $input)\r\n }","variables": {"input": {"lightlytics_internal_account_id":"${var.Lightlytics_internal_accountID}","role_arn":"arn:aws:iam::${var.cloud_account_id}:role/${coalesce(var.scan_role_name, "${var.environment}-lightlytics-role")}","account_type":"AWS","account_aliases":"","cloud_account_id":"${var.cloud_account_id}","stack_region":"${data.aws_region.current.name}","stack_id":"","init_stack_version":${var.init_stack_version}}}}'
EOF
  }
}
